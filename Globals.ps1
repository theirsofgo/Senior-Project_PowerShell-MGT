...
#region Get-Activation Status - Technet
...
#region Get-ListeningTCPConnections

#https://github.com/ratchetnclank/Powershell-Modules/blob/master/Get-ListeningTCPConnections.ps1
Function Get-ListeningTCPConnections
{
	...
	try
	{
		...
		{
			if ($Connection.address.AddressFamily -eq "InterNetwork") { $IPType = "IPv4" }
			else { $IPType = "IPv6" }
			
			$OutputObj = New-Object -TypeName PSobject
			$OutputObj | Add-Member -MemberType NoteProperty -Name "Local Address" -Value $connection.Address
			$OutputObj | Add-Member -MemberType NoteProperty -Name "Listening Ports" -Value $Connection.Port
			$OutputObj | Add-Member -MemberType NoteProperty -Name "IPV4 Or 6" -Value $IPType
			$OutputObj
		}
...
# Function RDP Enable
#https://gallery.technet.microsoft.com/scriptcenter/Remotely-Enable-RDP-855c3842
#Curently not functioning properly
...
#region Get-Uptime
#https://technet.microsoft.com/en-us/magazine/2008.12.heyscriptingguy.aspx
function Get-Uptime
{
	param ($ComputerName = "localhost")
...
### Address Calcuator - source functions : http://www.indented.co.uk/index.php/2010/01/23/powershell-subnet-math/
